...
//				len = usb_get_out_buffer(1, &RxDataBuffer);
                ...
                uint8_t pcktVer = RxDataBuffer[0] & 0x03;
                uint8_t pcktLen = (RxDataBuffer[0] >> 2) & 0x2F;

                if(pcktVer == 0x00 && pcktLen > 1) {
                    uint8_t opcode = RxDataBuffer[1];
                                        
                    if (opcode==OP_GET_ATTR_VALUE) {
                        
                        uint16_t attrid = (RxDataBuffer[2] << 8) + RxDataBuffer[3];
                                                
                        if (attrid==ATTR_PRODUCT_NAME) {
                            
                            uint8_t len = strlen(PRODUCT_NAME) + 1;        // +1 for NULL
                            TxDataBuffer[0] = len+2 << 2; // packet length
                            memcpy(&TxDataBuffer[2], PRODUCT_NAME, len);
 
                        } else if (attrid==ATTR_PRODUCT_REVISION) {
                            
                            uint8_t len = strlen(PRODUCT_REVISION) + 1;        // +1 for NULL
                            TxDataBuffer[0] = len+2 << 2; // packet length
                            memcpy(&TxDataBuffer[2], PRODUCT_REVISION, len);
                            
                        } else if (attrid==ATTR_PRODUCT_SERIAL) {

                            uint8_t len = strlen(PRODUCT_SERIAL) + 1;        // +1 for NULL
                            TxDataBuffer[0] = len+2 << 2; // packet length
                            memcpy(&TxDataBuffer[2], PRODUCT_SERIAL, len);

                        } else if (attrid==ATTR_FIRM_VERSION) {

                            uint8_t len = strlen(FIRM_VERSION) + 1;        // +1 for NULL
                            TxDataBuffer[0] = len+2 << 2; // packet length
                            memcpy(&TxDataBuffer[2], FIRM_VERSION, len);

                        } else {
                            TxDataBuffer[0] = 0x02 << 2; // packet length
                            TxDataBuffer[1] = 0x01 << 2; // Return error code
                        }
                        
                		memcpy(usb_get_in_buffer(1), TxDataBuffer, EP_1_IN_LEN);
                    	usb_send_in_buffer(1, EP_1_IN_LEN);

                    } else if (opcode==OP_SET_ATTR_VALUE) {
